...
CITY *city=new CITY[11];//the object of city;
...
int main(int argc , char *argv[])
{
	...
	for(int i=0;i<11;i++)
	{
		...
		if(city[i].name==0)//number of city < 11
		{
			break;
		}
		N+=1;
	...
	//number of city > 11 
	//resize the array 
	while(f>>n_in>>x_in>>y_in) 
	{
		if(n_in==0)//number of city
		{
			break;
		}
		//resize the array 
		CITY *temp = new CITY[N];
		for(int i=0;i<N;i++)
		{
			temp[i].name=city[i].name;
			temp[i].x=city[i].x;
			temp[i].y=city[i].y;
		}
		delete [] city;
